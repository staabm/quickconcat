...
function is_file_in_scope( $file, $assetRoot){
    $realpath = realpath($file);
    return ($file && is_file($realpath) && strpos($realpath, $assetRoot) === 0) ? $realpath : false;
}
...
foreach ( $files as $idx => $file ) {
    ...
    foreach($assetRoots as $assetRoot) {
        $realpath = $assetRoot . ltrim($file, '.~/\\');
    
        // santize url params: make sure the file which will be served is contained within an appRoot
        if ($actualPath = is_file_in_scope($realpath, $assetRoot)) {
            $files[$idx] = $actualPath;
            $found = true;
            break;
        }
    }
    // we allow only certain filetypes
    if( !preg_match( '/\.(js|html|css)$/', $file ) || !$found ){
        unset($files[$idx]);
    }
}
...
foreach ( $files as $file ) {
    ...
    if( $ftype === "css" ){
        ...
        $prefix = $pubroot . ltrim(dirname($file), "/") . "/";
